...
for l in dict_of_plans: 
    ...
    p.add_run("Аннотация к плану научной деятельности").add_break();
    p.add_run("Аннотация к программе итоговой аттестации").add_break();

    # * добавление второго параграфа
    p2 = document.add_paragraph()
    p2.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    p2.add_run('С полнотекстовыми версиями рабочих программ соответствующих дисциплин, программ практик, плана научной деятельности и программе итоговой аттестации можно ознакомиться в ЭБС СибАДИ. ')
    ...
    for disc_or_facult in discs_facults:
        ...
        p_disc_facult_p1.add_run('Дисциплина относится к образовательному компоненту. ')
    ...
    for practice in practices:
        ...
        p_practice_p2.add_run('Рабочей программой практики предусмотрены следующие виды контроля: промежуточная аттестация с оценкой. В программе практики приведено учебно-методическое, информационное обеспечение и материально-техническое обеспечение практики, оценочные и методические материалы.')
    ...
    # *   ----- block : АННОТАЦИЯ к плану научной деятельности -----      * #

    science_title = document.add_paragraph()
    science_title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    science_title.style = document.styles['Normal']
    science_title.add_run("Аннотация к плану научной деятельности").bold = True;

        # *   ----- block : ТЕЛО к плану научной деятельности -----      * #
    body_science = document.add_paragraph();
    body_science.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY;
    body_science.style = document.styles["Normal"];
    body_science.add_run("План научной деятельности образовательной программы высшего образования - программы подготовки научных и научно-педагогических кадров в аспирантуре является примерным и включает план выполнения научного исследования, план подготовки диссертации, план подготовки публикаций, в которых излагаются основные научные результаты диссертации, план прохождения промежуточной и итоговой аттестации, перечень этапов освоения научного компонента программы аспирантуры, распределение указанных этапов по курсам и форму контроля их выполнения. ");
        # *   ----- end : ТЕЛО к плану научной деятельности -----      * #

    # *   ----- end : АННОТАЦИЯ к плану научной деятельности -----      * #


    # *   ----- end : АННОТАЦИЯ к программе Итоговой аттестации -----      * #

    last_exam_title = document.add_paragraph()
    last_exam_title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    last_exam_title.style = document.styles['Normal']
    last_exam_title.add_run("Аннотация к программе ИА").bold = True;

        # *   ----- block : ТЕЛО к программе Итоговой аттестации -----      * #
    body_last_exam = document.add_paragraph();
    body_last_exam.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY;
    body_last_exam.style = document.styles["Normal"];
    body_last_exam.add_run("Итоговая аттестация по программе аспирантуры по научной специальности проводится в форме оценки диссертации на соответствие критериям, установленным Федеральным законом \"О науке и государственной научно-технической политике\" в \"Положении о присуждении ученых степеней\", утвержденным Постановлением Правительства Российской Федерации от 24.09.2013 № 842. Итоговая аттестация является обязательной. ");
        # *   ----- end : ТЕЛО к программе Итоговой аттестации -----      * #
